...
(function () {
	...
	window.addEventListener(cen, (event) => {
		...
		window.dispatchEvent(
			new CustomEvent(cen + 1, {
				detail: { result: handle(event.detail), from: "Content scripts" },
			}),
		);
	});
	...
	const inlineScript = (exchangeData) => {
		...
		const sendmsg = (data) =>
			window.dispatchEvent(
				new CustomEvent(cen, { detail: { data: data, from: "Page scripts" } }),
			);
		window.page_data = 1;
		const simulate = () => {
		...
		window.addEventListener(cen + 1, (event) =>
			console.log("Page scripts receive:", event.detail),
		);
	};
